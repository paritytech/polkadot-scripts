...
export async function nominatorThreshold(api: ApiPromise) {
	...
	console.log(
		`${n.filter(({ stake }) => stake.lt(t)).length} stashes are below ${api
			.createType('Balance', t)
			.toHuman()}`
	);
}
...
export async function stakingStats(api: ApiDecoration<'promise'>, baseApi: ApiPromise) {
	...
	const traverseNominatorBags = async (until: number): Promise<[AccountId32, number]> => {
		const bagThresholds = api.consts.voterList.bagThresholds.map((x) =>
	...
	// nominator stake
	{
		...
		console.log(
			`nominator stake: min-intention-threshold: ${b(minIntentionThreshold)} / min-electing = ${b(
				minElectingThreshold
			)} / min-active: ${b(minActiveThreshold)}`
		);
	}
	...
	// nominator count
	{
		...
		console.log(
			`nominator count: intentions: ${intentionCount} / electing: ${electingCount} / active: ${activeCount}`
		);
		console.log(
			`nominator count: max intention: ${intentionMax} / max electing ${electingMax} / max active: ${activeMax} `
		);
	}
	...
	// validator stake
	{
		...
		console.log(
			`validator stake: min-intention-threshold: ${b(minIntentionThreshold)} / min-electing: ${b(
				minElectableThreshold
			)} / min-active: ${b(minActiveThreshold)}`
		);
	}
	...
	// validator count
	{
		...
		console.log(
			`validator count: intentions: ${intentionCount} / electable: ${electableCount} / active: ${activeCount}`
		);
		console.log(
			`validator count: max intention: ${intentionMax} / max electable: ${electableMax} / max active: ${activeMax} `
		);
	}
	...
	// misc stake related stuff
	{
		...
		const validatorActiveStake = stakers
			.map((x) => x[1].own.toBn())
			.reduce((acc, x) => (acc = acc.add(x)));
		...
		const nominatorActiveStake = Array.from(assignments.values()).reduce(
			(acc, x) => (acc = acc.add(x))
		);
		const totalStaked = validatorTotalStake.add(nominatorTotalStake);
		console.log(
			`total issuance ${b(totalIssuance)} / staked ${b(totalStaked)} (${totalStaked
				.mul(new BN(100))
				.div(totalIssuance)}%) / active staked ${b(totalActiveStake)} (${totalActiveStake
				.mul(new BN(100))
				.div(totalIssuance)}%)`
		);
		console.log(
			`validator total stake = ${b(validatorTotalStake)} / validatorActiveStake = ${b(
				validatorActiveStake
			)}`
		);
		console.log(
			`nominator total stake = ${b(nominatorTotalStake)} / validatorActiveStake = ${b(
				nominatorActiveStake
			)}`
		);
	}
}
