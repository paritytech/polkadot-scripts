...
export async function chillOther(
	api: ApiPromise,
	account: KeyringPair,
	sendTx: boolean,
	noDryRun: boolean,
	...
	console.log(
		`📣 current status is ${await api.query.staking.counterForNominators()} / ${await api.query.staking.maxNominatorsCount()} nominators -- ${await api.query.staking.counterForValidators()} / ${await api.query.staking.maxValidatorsCount()} validators`
	);
	...
	if (noDryRun && sendTx) {
		...
		if (success && sendTx) {
			...
			success
				...
				: console.log(`warn: dry-run failed. not submitting anything.`);
		}
	}
}

async function buildChillTxs(
	api: ApiPromise,
	threshold: BN,
	chillThreshold: Percent,
	maybeLimit?: number
): Promise<SubmittableExtrinsic<'promise', ISubmittableResult>[]> {
	let allVotes = 0;
	const AllNominatorsRawPromise = (await api.query.staking.nominators.entries()).map(
		async ([stashKey, nomination]) => {
	...
	const allNominators = allNominatorsRaw.filter(({ ctrl, stake, ledger }) => {
	...
	const ejectedStake = toRemoveAll.length
		? toRemoveAll.map(({ stake }) => stake).reduce((prev, current) => (prev = current.add(prev)))
		: new BN(0);
	...
	console.log(
		`📊 a total of ${toRemoveAll.length} accounts with sum stake ${api
			.createType('Balance', ejectedStake)
			.toHuman()} (from the ${
			allNominators.length
		} total and ${allVotes} votes) are below the nominator threshold..`
	);
	...
	console.log(
		`\t.. thus ${Math.min(
			maxChillable,
			toRemoveAll.length
		)} can be chilled to stay below the ${chillThreshold.toHuman()} limit..`
	);
	...
	return toRemoveFinal.map(({ ctrl, stake }) => {
		console.log(
			`will chill ${ctrl.toHuman()} with stake ${api.createType('Balance', stake).toHuman()}`
		);
