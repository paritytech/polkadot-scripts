...
export async function stateTrieMigration(
	api: ApiPromise,
	account: KeyringPair,
	itemLimit: number,
	sizeLimit: number,
	...
	assert(
		// @ts-ignore
		maxLimits.unwrap()['size_'].gte(new BN(sizeLimit)) &&
			maxLimits.unwrap()['item'].gte(new BN(itemLimit)),
	...
	async function tryWithBackOff() {
		...
		while (repeat) {
			...
			console.log(
				`🌵 dry-run of transaction: success = ${success} ${dryRunOutcome.asOk.toString()}`
			);
			if (success) {
				...
				dryRunOutcome.isOk &&
				dryRunOutcome.asOk.isErr &&
				dryRunOutcome.asOk.asErr.isModule &&
				dryRunOutcome.asOk.asErr.asModule.error.eq(3)
			) {
				...
				div *= 2;
				console.log(
					`🖖 halving the number of items to migrate from ${itemLimit} to ${itemLimit / div}`
				);
				if (itemLimit / div < 1) {
